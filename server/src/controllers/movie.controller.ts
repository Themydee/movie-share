...
export const addMovie = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const movie = new Movie({
      ...
      review: cleanReview,
      recommendedBy: (req as any).user.userId, // ✅ from verifyToken
...
export const getUserMovie = async (req: Request, res: Response): Promise<void> => {
  try {
    const userId = req.params.userId;
    const movies = await Movie.find({ recommendedBy: userId });
